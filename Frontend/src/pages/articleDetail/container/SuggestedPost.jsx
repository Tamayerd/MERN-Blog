...
const SuggestedPost = ({ className, header, posts = [], tags }) => {
  return (
    <div
      className={`w-full shadow-[0_35px_60px_-15px_rgba(0,0,0,0.3)] rounded-lg p-4 ${className} `}
    >
      ...
      <div className="flex flex-wrap gap-x-2 gap-y-2 mt-4">
        {tags.map((item, index) => (
          <Link
            key={index}
            to="/"
            className="inline-block rounded-md px-3 py-1.5 bg-black font-roboto text-xs text-white md:test-sm"
          >
            {item}
          </Link>
        ))}
